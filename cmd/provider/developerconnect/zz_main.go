...
func main() { //nolint:gocyclo // easier to follow as a unit
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-gcp-developerconnect",
		Cache: cache.Options{
	...
	if startWebhooks {
		kingpin.FatalIfError(clustercontroller.SetupWebhookWithManager_developerconnect(mgr), "Cannot setup cluster-scoped webhooks")
		kingpin.FatalIfError(namespacedcontroller.SetupWebhookWithManager_developerconnect(mgr), "Cannot setup namespaced webhooks")
	}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_developerconnect(mgr, clusterOpts), "Cannot setup cluster-scoped GCP controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_developerconnect(mgr, namespacedOpts), "Cannot setup namespaced GCP controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_developerconnect(mgr, clusterOpts), "Cannot setup cluster-scoped GCP controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_developerconnect(mgr, namespacedOpts), "Cannot setup namespaced GCP controllers")
	}

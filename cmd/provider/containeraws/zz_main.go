...
func main() { //nolint:gocyclo // easier to follow as a unit
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_containeraws(mgr, clusterOpts), "Cannot setup cluster-scoped GCP controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_containeraws(mgr, namespacedOpts), "Cannot setup namespaced GCP controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_containeraws(mgr, clusterOpts), "Cannot setup cluster-scoped GCP controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_containeraws(mgr, namespacedOpts), "Cannot setup namespaced GCP controllers")
	}

...
import (
	...
	providerinit "github.com/upbound/provider-gcp/v3/cmd/provider"
	"github.com/upbound/provider-gcp/v3/config"
	resolverapis "github.com/upbound/provider-gcp/v3/internal/apis"
	"github.com/upbound/provider-gcp/v3/internal/bootcheck"
	"github.com/upbound/provider-gcp/v3/internal/clients"
	clustercontroller "github.com/upbound/provider-gcp/v3/internal/controller/cluster"
	namespacedcontroller "github.com/upbound/provider-gcp/v3/internal/controller/namespaced"
	"github.com/upbound/provider-gcp/v3/internal/features"
	"github.com/upbound/provider-gcp/v3/internal/version"
)
...
func main() { //nolint:gocyclo // easier to follow as a unit
	...
	kingpin.FatalIfError(clusterapis.AddToScheme(scheme), "Cannot add cluster-scoped GCP APIs to scheme")
	kingpin.FatalIfError(resolverapis.BuildScheme(clusterapis.AddToSchemes), "Cannot register the cluster-scoped GCP APIs with the API resolver's runtime scheme")
	kingpin.FatalIfError(namespacedapis.AddToScheme(scheme), "Cannot add namespace-scoped GCP APIs to scheme")
	kingpin.FatalIfError(resolverapis.BuildScheme(namespacedapis.AddToSchemes), "Cannot register the namespace-scoped GCP APIs with the API resolver's runtime scheme")
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-gcp-firestore",
		Cache: cache.Options{
	...
	if cmd == initCmd.FullCommand() {
		kingpin.FatalIfError(providerinit.RunStorageVersionMigration(ctx, logr, mgr, "firestore"), "Cannot run storage version migrator")
		return
	}
	...
	if *enableChangeLogs {
		...
		clo := xpcontroller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-upjet-gcp:%s", version.Version))),
		}
	...
	if startWebhooks {
		kingpin.FatalIfError(clustercontroller.SetupWebhookWithManager_firestore(mgr), "Cannot setup cluster-scoped webhooks")
		kingpin.FatalIfError(namespacedcontroller.SetupWebhookWithManager_firestore(mgr), "Cannot setup namespaced webhooks")
	}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_firestore(mgr, clusterOpts), "Cannot setup cluster-scoped GCP controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_firestore(mgr, namespacedOpts), "Cannot setup namespaced GCP controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_firestore(mgr, clusterOpts), "Cannot setup cluster-scoped GCP controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_firestore(mgr, namespacedOpts), "Cannot setup namespaced GCP controllers")
	}
